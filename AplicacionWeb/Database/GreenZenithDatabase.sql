...
CREATE TABLE PUser (
    ...
    PasswordPUser NVARCHAR(30) NOT NULL
);
...
CREATE TABLE AdministratorAccess (
    UserID INT PRIMARY KEY,
    FOREIGN KEY (UserID) REFERENCES PUser(ID) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE MessageReciever (
    ...
    FOREIGN KEY (SenderID) REFERENCES MessageSender(ID) ON DELETE CASCADE ON UPDATE CASCADE
);
